...
class AppointmentRepository {
  ...
  Future<List<AppointmentChat>> getAppointmentChats() async {
    final response =
        await _dio.get('$APPOINTMENT_CHAT_URL/', options: getOptions());
    final chatsJson = response.data as List<dynamic>;
    final chats = chatsJson
        .map((chatJson) => AppointmentChat.fromJson(chatJson))
        .toList();
    return chats;
  }

  Future<List<Appointment>> getAppointments(bool completed) async {
    final response = await _dio.get('$APPOINTMENTS_URL/?done=$completed',
        options: getOptions());
  ...
  Future<AppointmentChat> getAppointmentChat(int id) async {
    final response =
        await _dio.get('$APPOINTMENT_CHAT_URL/$id/', options: getOptions());
  ...
  Future<AppointmentChat> saveAppointmentChat(AppointmentChat chat) async {
    final response = await _dio.post('$APPOINTMENT_CHAT_URL/',
        options: postOptions(), data: chat.toJsonMap());
  ...
  Future<ChatMessage> sendMessage(FormData data) async {
    final response = await _dio.post('$CHAT_MESSAGES_URL/',
        options: postOptions(), data: data);
  ...
  Future<List<ChatMessage>> getAppointmentChatMessages(
      int appointmentChatId) async {
    final response = await _dio.get(
        '$APPOINTMENT_CHAT_URL/$appointmentChatId/messages/',
        options: getOptions());
  ...
  Future<Appointment> updateAppointment(Appointment appointment) async {
    ...
    final response = await _dio.put('$APPOINTMENTS_URL/${appointment.id}/',
        options: putOptions(), data: appointment.toJson());
