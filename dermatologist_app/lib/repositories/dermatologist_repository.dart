...
import 'package:dermatologist_app/models/dermatologist.dart';
import 'package:dermatologist_app/api_connection/endpoints.dart';
import 'package:dermatologist_app/api_connection/con_options.dart';
...
class DermatologistRepository {
  ...
  Future<List<Dermatologist>> getDermatologists() async {
    final response =
        await _dio.get('$DERMATOLOGISTS_URL/', options: getOptions());
  ...
  Future<Dermatologist> getDermatologistById(int id) async {
    final response =
        await _dio.get('$DERMATOLOGISTS_URL/$id/', options: getOptions());
  ...
  Future<List<Dermatologist>> getNearbyDermatologists(
      latitude, longitude) async {
    final response = await _dio.get(
        '$DERMATOLOGISTS_URL/nearby?latitude=$latitude&longitude=$longitude',
        options: getOptions());
    final dermatologistsJson = response.data as List<dynamic>;
    final dermatologists = dermatologistsJson
        .map((json) => Dermatologist.fromJson(json as Map<String, dynamic>))
